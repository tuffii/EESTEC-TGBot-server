...
const EESTECPrivilegeSearchValidator = new Validator().Default.Object()
  .Required()
  .Exact({
    id: new Validator().Default.Numeric().Number().Integer().Min(1),
    privilege: new Validator().Default.String().MinLength(1),
...
export default class PrivilegeService {
  public static async Get(
    searchParameters: EESTECPrivilegeSearch
  ): Promise<Array<EESTECPrivilegeOutput>> {
    const Validator = ServerCore.Engine.Module.Classes.Validator
    Validator.Strict(searchParameters, EESTECPrivilegeSearchValidator)

    const IDToSearch = searchParameters.id
    const IsIDToSearchExist =
      !Validator.TypeGuard.Default.IsUndefined(IDToSearch)
    const privilegeToSearch = searchParameters.privilege
    const IsPrivilegeToSearchExist =
      !Validator.TypeGuard.Default.IsUndefined(privilegeToSearch)
    ...
    if (IsIDToSearchExist)
      EESTECPrivilegeMatchConditions.push(
        EESTECPrivilegeTable.Condition.Single.EqualTo(`id`, IDToSearch)
      )
    if (IsPrivilegeToSearchExist)
      EESTECPrivilegeMatchConditions.push(
        EESTECPrivilegeTable.Condition.Single.EqualTo(
          `privilege`,
          privilegeToSearch
        )
      )

    const EESTECPrivilegeMatchCondition =
      EESTECPrivilegeTable.Condition.Multiple.Every(
        ...EESTECPrivilegeMatchConditions
      )
    const MatchedEESTECPrivileges = await EESTECPrivilegeTable.Select(
      undefined,
      EESTECPrivilegeMatchCondition,
      false,
      undefined
    )
  ...
  public static async DoesExist(
    ...
    const IsMatchedEESTECPrivilegeExist =
      !Validator.TypeGuard.Default.IsEmptyArray(MatchedEESTECPrivileges)
